...
def match(bbox: torch.Tensor,
    ...
    # 1. 每个anchor找gt
    best_gt_iou, best_gt_idx = jac.max(dim=1)

    # 2. 每个gt找anchor（强制正样本）
    best_anchor_iou, best_anchor_idx = jac.max(dim=0)

    match_gt_idx = best_gt_idx.clone()

    # 强制匹配（关键）
    match_gt_idx[best_anchor_idx] = torch.arange(gt_num)

    positive_sample = best_gt_iou >= pos_thresh
    negative_sample = best_gt_iou < neg_thresh

    # 强制正样本
    positive_sample[best_anchor_idx] = True
    negative_sample[best_anchor_idx] = False
    ...
    # # ————————————
    # print(f'positive_num: {(cls_targets > 0).sum()}')
    # # ————————————
