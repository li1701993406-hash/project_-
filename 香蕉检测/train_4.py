    ...
    for epoch in range(epoch):
        ...
        for images, labels in dataloader:
            ...
            #————————————————
                ...
                # 获得并拼接多尺度anchor
            anchors = []
            anchor_size = [[32, 64], [64, 128], [128, 256]]
            stride = [4, 16, 64]

            for feature, a_size, s in zip(features, anchor_size, stride):
            ...
            # print(f'cls_prediction.shape: {cls_prediction.shape}')
            # print('-' * 10)
            # #————————————————————
            ...
            # #————————————
            # print(f'positive_num: {(batch_bbox_targets > 0).sum()}')
            # #————————————
            ...
            # #————————————————
            # with torch.no_grad():
            #     prob = torch.softmax(cls_prediction.reshape(-1, 2), dim=-1)
            #
            #     pos_mask = batch_positive_sample.reshape(-1)
            #
            #     pos_scores = prob[pos_mask][:, 1]
            #
            #     if pos_scores.numel() > 0:
            #         print(
            #             "pos mean:", pos_scores.mean().item(),
            #             "pos max:", pos_scores.max().item()
            #         )
            # #————————————————
            ...
            total_loss_history.append(running_total_loss)
            cls_loss_history.append(running_cls_loss)
            bbox_loss_history.append(running_bbox_loss)
        ...
        print(f'第{epoch + 1}轮训练循环结束')
        print(f'准备进行下一轮循环')


        # for i in range(100):
        #     print(f'{100 - i}秒后进入下一轮循环', end='\n')
        #     time.sleep(1)
